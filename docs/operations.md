...
Events:
  ...
  Warning  GuardRequired  22s    hcloud-operator  spec no longer lists "cpx21", which the server runs; it would be resized to "cpx31", the first type the list does name. Resizing powers the server off and back on, so set spec.allowDowntime: true to apply it.
```
